...
#
# When n goes to infinity and x has a correct value (ie x is in its domain of convergence D),
# U(n, x) goes to a finite limit m depending on x.
#
# Usually given x we try to find m. Here we will try to find x (x real, 0 < x < 1)
# when m is given (m real, m > 0).
#
# Let us call solve the function solve(m) which returns x such as U(n, x) goes to
